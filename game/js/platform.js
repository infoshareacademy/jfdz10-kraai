...
const gameStart = () => {
  ...
  const htmlObjects = {
    ...
    getCandyPositions() {
      return Array.from(this.getCandyElements())
        .map(element => ({
          width: element.offsetWidth,
          height: element.offsetHeight,
          left: element.offsetLeft,
  ...
  const candyColision = () => {
    htmlObjects.getCandyPositions().forEach(candy => {
      if (checkCollisionX(htmlObjects.getCharacterPosition(), candy) && checkCollisionY(htmlObjects.getCharacterPosition(), candy)) {
        score += 1;
        htmlObjects.scoreCounter.textContent = score;
